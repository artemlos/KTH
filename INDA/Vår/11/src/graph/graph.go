...
// added a change to transverse to ensure that nodes point to a parrent node. Artem, 2015.04.27
package graph
...
func BFS(g Iterator, v int, visited []bool, action func(w int), rowpointer *[]int) {
	traverse(g, v, visited, action, bfs, *rowpointer)
}
...
// When the algorithm arrives at a node w for which visited[w] is false,
// action(w) is called and visited[w] is set to true.
func DFS(g Iterator, v int, visited []bool, action func(w int), rowpointer *[]int) {
	traverse(g, v, visited, action, dfs, *rowpointer)
}
...
func traverse(g Iterator, v int, visited []bool, action func(w int), order int, rowpointer []int) {
	var queue []int
